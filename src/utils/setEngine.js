...
const validateAnticipationBranch = (branch, path, { allowWait = false } = {}) => {
  ...
  if (allowWait) {
    if (branch.esperar_horas !== null && (!Number.isInteger(branch.esperar_horas) || branch.esperar_horas < 1)) {
      throw contractError(`Contrato SET inválido: ${path}.esperar_horas debe ser null o un entero positivo.`);
    }
  }
};
...
export const parseAndValidateSetEngineResponse = (text, project) => {
  ...
  if (contract.version !== 'set_engine_v1') throw contractError('Contrato SET inválido: versión incorrecta.');
  requireEnum(contract.modo, MODES, 'modo');
  if (contract.modo !== 'analisis') throw contractError('Contrato SET inválido: este flujo requiere modo analisis.');
  ...
  if (contract.decision.respuesta !== null) {
    ...
    if (!contract.decision.respuesta.piezas.length) throw contractError('Contrato SET inválido: la respuesta debe contener al menos una pieza.');
    ...
    contract.decision.respuesta.piezas.forEach((piece, index) => {
      ...
      if (piece.tipo === 'recurso') {
        if (piece.contenido !== null) throw contractError(`Contrato SET inválido: ${path}.contenido debe ser null para recursos.`);
        ...
        if (piece.recurso_id !== null) throw contractError(`Contrato SET inválido: ${path}.recurso_id debe ser null.`);
      }
    });
  }
  ...
  if (contract.estado_inferido.nivel_compromiso !== 'no_determinado' && !contract.estado_inferido.senales.length) {
    throw contractError('Contrato SET inválido: el nivel de compromiso requiere señales observables.');
  }
