            ...
            {links.map(({ to, icon: Icon, label, badge }) => (
              <Link key={to} to={to}
                className={`relative flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                  isActive(to)
                    ? to === '/leads-reales' ? 'text-white' : 'bg-accent-coral text-white'
        ...
        <div className="flex md:hidden items-center justify-around h-16">
          {links.map(({ to, icon: Icon, label, badge }) => (
            <Link key={to} to={to}
              ...
              {badge > 0 && (
                <span className="absolute top-0 right-0 text-xs bg-red-500 text-white rounded-full w-4 h-4 flex items-center justify-center font-bold leading-none">
                  {badge}
                </span>
              )}
            </Link>
